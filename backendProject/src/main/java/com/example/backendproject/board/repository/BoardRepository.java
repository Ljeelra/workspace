...
public interface BoardRepository extends JpaRepository<Board,Long> {
    // 특정 User의 게시글만 조회Add commentMore actions
    //@Query(value = "SELECT * FROM board WHERE user_id = :userId", nativeQuery = true)  //이거 사용안해도 되는데 예시로 네이티브쿼라로 작성한거임Add commentMore actions
    List<Board> findByUserId(Long userId);

    //보드 엔티티를 기준으로 조회하되Add commentMore actions
