...
public class JwtTokenFilter extends OncePerRequestFilter {
    //JwtTokenFileter 모든 HTTP 요청을 가로채서 JWT 토큰을 검사하는 필터 역할
    ...
    protected void doFilterInternal(HttpServletRequest request, //http 요청
        ...
        if(accessToken != null && jwtTokenProvider.validateToken(accessToken)) {
            ...
            String method = request.getMethod(); //"GET, POST, PUT" 등 어떤 메서드인지 알 수 있음
    ...
    private UsernamePasswordAuthenticationToken getAuthentication(String token) {
        ...
        return new UsernamePasswordAuthenticationToken(
                userDetails, //사용자 정보
                null, // 이미 인증했기 때문에 인증정보가 필요없어서 null
